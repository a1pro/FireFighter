    ...
    launchImageLibrary({mediaType: 'photo', quality: 0.5}, response => {
      ...
      if (type === 'floor') {
        setFloorDetails(prev => ({
          ...prev,
          [number]: {...prev[number], image},
        }));
      } else {
        setBasementDetails(prev => ({
          ...prev,
          [number]: {...prev[number], image},
        }));
      }
      ...
      floorsToSubmit.forEach(floor => {
        ...
        if (img) {
          formData.append('floor_image[]', {
            ...
            name: img.fileName || `floor_${num}.jpg`,
          });
        }
      });

      // Determine basements to submit
      const basementsToSubmit =
        selectedBasements.length > 0
          ? selectedBasements
      ...
      basementsToSubmit.forEach(bs => {
        ...
        if (img) {
          formData.append('basement_image[]', {
            uri: img.uri,
            type: img.type || 'image/jpeg',
            name: img.fileName || `basement_${num}.jpg`,
          });
        }
      });
        ...
        return (
          <SafeAreaView>
            <ScrollView contentContainerStyle={{flexGrow: 1}}>
              <View
                style={[styles.container, {paddingBottom: 30, paddingTop: 30}]}>
                {/* Business Address */}
                ...
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('MapScreen', {
                      businessAddress: values.buildingAddress,
                    })
                  }
                ...
                {selectedFloors.map((floor, index) => (
                  <View key={index} style={{marginTop: 20}}>
                    ...
                    {floorDetails[floor.floorNumber]?.image && (
                      <Image
                        source={{
                          uri: floorDetails[floor.floorNumber].image.uri,
                        }}
                ...
                {selectedBasements.map((basement, index) => (
                  <View key={index} style={{marginTop: 20}}>
                    ...
                    <TouchableOpacity
                      style={styles.btn}
                      onPress={() =>
                        selectImage('basement', basement.basementNumber)
                      }>
                    ...
                    {basementDetails[basement.basementNumber]?.image && (
                      <Image
                        source={{
                          uri: basementDetails[basement.basementNumber].image
                            .uri,
                        }}
                        style={{width: 100, height: 100, marginTop: 10}}
                      />
