...
const BuildingDetails = ({ route }) => {
  ...
  const sendOtpForEditBuilding = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err.message || 'Something went wrong while sending OTP.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', err.message || 'Something went wrong while verifying OTP.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: '#18222C', paddingHorizontal: 10 }]}>
      <View style={{ paddingVertical: 20 }}>
        <Text style={[styles.h1, { textAlign: 'center', color: '#fff' }]}>Business Details</Text>
      </View>
